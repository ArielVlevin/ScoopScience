    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-primary to-primary-foreground">
      {!iceCreamType && (
        <div className="bg-background p-8 rounded-lg shadow-lg">
          <h1 className="text-3xl font-bold mb-4">What type of ice cream would you like to make?</h1>
      ...
      {iceCreamType && (
        <div className="bg-background p-8 rounded-lg shadow-lg w-full max-w-4xl">
          <div className="flex items-center justify-between mb-4">
            ...
            <h1 className="text-3xl font-bold">Choose your {iceCreamType} flavor</h1>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {flavors[iceCreamType]?.map((flavor) => (
              <div
                ...
                className={`bg-muted p-4 rounded-lg shadow-lg hover:bg-muted/50 transition-colors cursor-pointer ${
                  selectedFlavor === flavor.name ? "border-2 border-primary" : ""
                }`}
                onClick={() => setSelectedFlavor(flavor.name)}
              >
                <img src="/placeholder.svg" alt={flavor.name} width={200} height={200} className="rounded-lg mb-2" />
          ...
          {selectedFlavor && (
            <div className="mt-8 flex flex-col items-center">
              ...
              <Link to='../recipes/new' state={{ type: iceCreamType, flavor: selectedFlavor }} className="btn btn-primary">
                Create Recipe
              </Link>
            </div>
          )}
