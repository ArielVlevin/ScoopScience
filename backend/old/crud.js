...
export async function getLastInsertedID(categoryName) {
  try {
    const result = await Ingredient.findOne({ category: categoryName })
      .sort({ _id: -1 })
      .exec();
    return result?._id || 0;
  } catch (error) {
...
export async function getLastRecipeID() {
  try {
    const result = await Recipe.findOne().sort({ _id: -1 }).exec();
    return result?._id || 30000;
  } catch (error) {
    console.error("Error retrieving last inserted ID:", error);
    throw new Error("Failed to retrieve last inserted ID");
  }
